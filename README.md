...
## Future work
 ...
 - Exploring the dataset profiling report, I noticed a data imbalance warning, which makes the model completely biased, impacting the relevance and the correlations between features and target outputs.In return, it will impact the model accuracy overall and it could lead to wrong assemptions. <br> To enhance the training data and remove the imbalance issue, we can apply the  _Resampling_ or _Ensembling_ methods. <br> It would be great to try those fixes and compare results with the one generated by AuoML.<br>
 - I also want to see how relevant is the data cleaning performed by AutoML. This experiment aims to gain time/performance and comparing the accuracy with the old one.<br>

Finally, I am eager to fix the cloud shell error about storage limitations when running the model.
